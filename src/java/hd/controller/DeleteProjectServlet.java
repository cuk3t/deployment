...
public class DeleteProjectServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (user != null && user.getUserId() == project.getProfessionalUserId().getUserId()) {
                project.setStatus(Constant.STATUS_DELETED);
                projectJpa.edit(project);
                url = "MainProfessionalServlet?btnAction=load&txtUserId=" + project.getProfessionalUserId().getUserId();
            }
            response.sendRedirect(url);
        } catch (Exception e) {
            log("Error at DeleteProjectServlet: " + e.getMessage());
        }
    }
